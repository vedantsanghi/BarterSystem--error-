...
export default class ItemRequestScreen extends Component {
  ...
  getData=async()=> {
    var valueInINR;
   await fetch(
      "http://data.fixer.io/api/latest?access_key=0cd9c977152ce4db651faea8f3bd02c1&format=1"
    )
      .then((response) => {
      ...
      .then((responseData) => {
        ...
        valueInINR =
          (this.state.value / rates[this.state.CurrencyCode]) * rates.INR;

        console.log(valueInINR);
        this.setState({
          valueInINR: valueInINR,
        });
        console.log(this.state.valueInINR);
      });
      return valueInINR
  }
  ...
  addRequest = async (itemName, description) => {
    ...
    db.collection("requested_items").add({
      ...
      valueInINR: this.state.valueInINR
    });
        ...
        <View style={{ flex: 1 }}>
          ...
          <ScrollView>
            <KeyboardAvoidingView style={styles.keyBoardStyle}>
              ...
              <TextInput
                style={styles.formTextInput}
                placeholder={"Item Value in Country Currency Code"}
